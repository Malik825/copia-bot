...
export default function SignUp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    setTelemetryLogs([
      "SECURE_REG // Provisioning new account record...",
      "CIPHER_CHK // Wrapping user credentials in secure hash...",
    ]);
    ...
    try {
      ...
      if (json.success && json.data) {
        ...
        setTelemetryLogs((prev) => [
          ...prev,
          "FAIL_VAL   // License registry rejected payload.",
        ]);
      }
    } catch (err) {
      ...
      setTelemetryLogs((prev) => [
        ...prev,
        "ERR_CONN   // Parallel routing link interrupted.",
      ]);
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-md relative z-10 space-y-8">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="font-mono text-[9px] tracking-[2.5px] uppercase text-muted-foreground">
            GET AUTOMATED ROUTER CONNECTION
          </p>
        </div>
        ...
        <div className="bg-card border border-border p-8 md:p-10 rounded-xl relative shadow-lg backdrop-blur-xl dark:bg-black/50 transition-all duration-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            >
              {loading ? (
                ...
                <>
                  <Key className="w-3.5 h-3.5" /> Provision Terminal License <ArrowRight className="w-3.5 h-3.5" />
                </>
              )}
            </button>
          </form>
          ...
          <div className="mt-8 pt-6 border-t border-border/40 text-center">
            <p className="font-mono text-[10px] text-muted-foreground">
              Already have an active license?{" "}
